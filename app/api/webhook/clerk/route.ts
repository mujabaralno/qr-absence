...
export async function POST(req: Request) {
    ...
    try {
      ...
      if (newUser) {
        const result = await (await clerkClient()).users.updateUserMetadata(id, {
          publicMetadata: { userId: newUser._id, approved: false, organization },
        });
  ...
  if (eventType === "user.updated") {
    const { id: updatedUserId, image_url, first_name, last_name, unsafe_metadata } = evt.data;

    const user = {
      firstName: first_name!,
      lastName: last_name!,
      photo: image_url,
      npm: unsafe_metadata.npm as string,
      angkatan: unsafe_metadata.angkatan as string,
    };
